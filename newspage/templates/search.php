	...
	<div class="row">
		...
		<?php 
			...
			if (isset($_GET['p'])) {
				...
				$limit = 20; // Gioi han so bai viet trong 1 trang
				$totalPage  = ceil($countPost / $limit); // Tong so trang khi tinh tong toan
				...
				$sql_get_news = "SELECT * FROM posts WHERE status = '1' AND titile LIKE '%$s%' OR keywords LIKE '%$s%' OR descr LIKE '%$s%' ORDER BY id_post DESC LIMIT  $start, $limit ";
				if ($db->num_rows($sql_get_news)) {
					foreach ($db->fetch_assoc($sql_get_news) as $data_post) {
						echo '
							<div class="col-md-3">
								<div class="thumbnail">
					...
					echo '<div class="well well-lg">Khong tim thay ket qua nao.</div>';
				}
			} else {
				echo '<div class="alert alert-danger">Vui long nhap tu khoa tim kiem</div>';
			}
		?>
	</div>
</div>